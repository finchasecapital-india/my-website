(() => {
  ...
  /* Shared Web3Forms post, all inquiries route to finchasecapital@gmail.com */
  const postLead = async (form) => {
    const res = await fetch("https://api.web3forms.com/submit", { method: "POST", body: new FormData(form) });
    const data = await res.json().catch(() => ({}));
    if (!data.success) throw new Error((data && data.message) || "send failed");
    return data;
  };
